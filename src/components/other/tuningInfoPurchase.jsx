...
function TuningInfoPurchase() {
  const { id } = useParams();
  const data = tuning.maindata.find((value) => value.id === parseInt(id));
  return (
    <PurchaseContainer>
        <img src={cart} alt="" />
        <PurchaseCarWrapper>
            <PurchaseLeftWrapper>
                <PurchaseLeftWrapperImg src={data.tuning.image} alt="" />
                <PurchaseLeftWrapperPriceBox>
                    <p>Purchase price</p>
                    <p style={{fontWeight: "600"}}>{data.tuning.price}</p>
                </PurchaseLeftWrapperPriceBox>
            </PurchaseLeftWrapper>

            <PurchaseRightWrapper>
                <SpaceBetween>
                    <h1 style={{fontWeight: '600'}}>{data.tuning.name}</h1>
                    <h1 style={{fontWeight: '600', color: "#006DAB", marginTop: '20px'}}>{data.tuning.price} W</h1>
                </SpaceBetween>
...
export default TuningInfoPurchase
