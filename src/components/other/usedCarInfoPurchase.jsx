...
function UsedCarInfoPurchase() {
  const { id } = useParams();
  const data = usedCar.maindata.find((value) => value.id === parseInt(id));
  return (
    <PurchaseContainer>
        <img src={cart} alt="" />
        <PurchaseCarWrapper>
            <PurchaseLeftWrapper>
                <PurchaseLeftWrapperImg src={data.used.image} alt="" />
                <PurchaseLeftWrapperPriceBox>
                    <p>Purchase price</p>
                    <p style={{fontWeight: "600"}}>{data.used.price}</p>
                </PurchaseLeftWrapperPriceBox>
            </PurchaseLeftWrapper>

            <PurchaseRightWrapper>
                <SpaceBetween>
                    <h1 style={{fontWeight: '600'}}>{data.used.name}</h1>
                    <h1 style={{fontWeight: '600', color: "#006DAB", marginTop: '20px'}}>{data.used.price} W</h1>
                </SpaceBetween>
...
export default UsedCarInfoPurchase
