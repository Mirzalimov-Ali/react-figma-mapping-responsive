...
function CaravanInfo() {
  const [index, setIndex] = React.useState(0);
  const { id } = useParams();
  const resultData = caravan.maindata.find((value) => value.id === parseInt(id));
  ...
  return (
    <CarInfoContainer>
      ...
      <LineWrappers>
        <div>
            <CarImg src={resultData.caravan.image} alt="" width={"500px"}/>
        </div>
        <InfoWrapper>
            <InfoWrapperLine1>
                <div>
                    <h3>{resultData.caravan.name}</h3>
                    <small>aid</small>
                </div>
                <p style={{fontSize: "30px", fontWeight: "600", color: "#006DAB"}}>{resultData.caravan.price}Won</p>
            </InfoWrapperLine1>
            <InfoWrapperLine/>
            <InfoWrapperLine2>
                <InfoWrapperBottom>
                    <p>Company</p>
                    <p style={{fontSize: "12px", opacity: "0.5"}}>{resultData.caravan.company}</p>
                </InfoWrapperBottom>
                <InfoWrapperBottom>
                    <p>People</p>
                    <p style={{fontSize: "12px", opacity: "0.5"}}>{resultData.caravan.people}</p>
                </InfoWrapperBottom>
                <InfoWrapperBottom>
                    <p>License type</p>
